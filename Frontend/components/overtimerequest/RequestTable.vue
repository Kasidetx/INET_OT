<template>
  <v-card outlined elevation="0" class="white rounded-xl border-light">
    <v-card-title class="pb-4 blue--text d-flex flex-wrap justify-space-between align-center">
      <span class="text-h6 font-weight-bold">รายการเอกสาร {{ filteredItems.length }} รายการ</span>

      <div v-if="showSelect && selected.length > 0" class="mt-2 mt-sm-0 w-full-mobile">
        <v-btn color="#D1392B" dark depressed @click="$emit('bulk-cancel')" class="rounded-lg px-6 subtitle-2 w-full-mobile"
          height="40">
          ...
          <div class="flex-grow-1">
            ...
            <div class="date-time-box rounded pa-2 mb-3">
              <div class="d-flex justify-space-between mb-1">
                ...
                <span class="caption font-weight-bold">{{ item.startDate }} <span class="primary--text">{{ item.startTime }}</span></span>
              </div>
              <div class="d-flex justify-space-between">
                <span class="caption grey--text">สิ้นสุด:</span>
                <span class="caption font-weight-bold">{{ item.endDate }} <span class="primary--text">{{ item.endTime }}</span></span>
              </div>
            </div>
