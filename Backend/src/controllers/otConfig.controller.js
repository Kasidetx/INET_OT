...
export const getOtConfigById = catchAsync(async (req, res) => {
    ...
    if (!otConfig) {
        throw { statusCode: 404, message: 'OT Config not found' };
    }
...
export const createOtConfig = catchAsync(async (req, res) => {
    ...
    const newOtConfig = await OtConfigModel.create({
        description, type_work_id, start_time, end_time, rate, hour, deduction_min, is_active
    });
...
export const updateOtConfig = catchAsync(async (req, res) => {
    ...
    if (!updated) {
        throw { statusCode: 404, message: 'OT Config not found or no changes made' };
    }
